/**
 * Elefante Letrado Script - Módulo de Diagnóstico e Monitor de Performance v1.0.2
 * Monitoramento isolado de FPS, Event-Loop Lag, Long Tasks, Mutações DOM e Desempenho.
 * v1.0.2: Adicionado detalhamento de mutações por tipo (childList, attributes, characterData) e Top Elementos Mutados.
 */
...
(function () {
  ...
  const PerformanceMonitorModule = {
    ...
    mutationCountTemp: 0,
    mutationTypesTemp: { childList: 0, attributes: 0, characterData: 0 },
    lastMutationBreakdown: { childList: 0, attributes: 0, characterData: 0, topTargets: [] },
    lastDomNodesCount: 0,
    ...
    enable(mode = 'NORMAL') {
      ...
      console.log(`[📊 Diagnostics v1.0.2] Monitor ativado no modo: ${this.mode}`);

      // Observador de Visibilidade da Aba
      ...
      // 2. Inicia Medição de Event-Loop Lag via setTimeout(0)
      let expectedLagTime = performance.now();
      const checkLag = () => {
        const now = performance.now();
        if (!document.hidden) {
          const lag = Math.max(0, now - expectedLagTime - 20);
    ...
    enableDeepFeatures() {
      ...
      // B. Observer Avançado de Mutações no DOM com Identificação de Elementos Afetados
      if (typeof MutationObserver !== 'undefined') {
        ...
        this.mutationObserver = new MutationObserver((mutations) => {
          ...
          for (let i = 0; i < mutations.length; i++) {
            const m = mutations[i];
            if (m.type === 'childList') this.mutationTypesTemp.childList++;
            else if (m.type === 'attributes') this.mutationTypesTemp.attributes++;
            else if (m.type === 'characterData') this.mutationTypesTemp.characterData++;
            ...
            if (target && target.nodeType === 1) { // Node.ELEMENT_NODE
              ...
              targetCounts.set(selector, (targetCounts.get(selector) || 0) + 1);
            }
          }
        });
        ...
        this.mutationSecIntervalId = setInterval(() => {
          ...
          this.lastMutationBreakdown = {
            childList: this.mutationTypesTemp.childList,
            attributes: this.mutationTypesTemp.attributes,
            characterData: this.mutationTypesTemp.characterData,
            topTargets
          };

          this.mutationCountTemp = 0;
          this.mutationTypesTemp = { childList: 0, attributes: 0, characterData: 0 };
          targetCounts.clear();
          this.updateHUD();
        }, 1000);
      }
    ...
    takeSnapshot() {
      ...
      this.snapshotsBuffer.push({
        ...
        mutationBreakdown: this.mode === 'DEEP' ? { ...this.lastMutationBreakdown } : null,
    ...
    clear() {
      ...
      this.mutationTypesTemp = { childList: 0, attributes: 0, characterData: 0 };
      this.lastMutationBreakdown = { childList: 0, attributes: 0, characterData: 0, topTargets: [] };
      this.updateHUD();
    ...
    exportJSON() {
      const report = {
        ...
        summary: {
          ...
          domMutationsPerSec: this.domMutationsPerSec,
          lastMutationBreakdown: this.mode === 'DEEP' ? this.lastMutationBreakdown : null
        },
    ...
    createHUD() {
      ...
      hud.style.cssText = `
        ...
        width: 250px; user-select: none; pointer-events: auto;
      `;
      ...
      hud.innerHTML = `
        ...
        <div id="ea-diag-hud-body">
          ...
          ${this.mode === 'DEEP' ? `
            ...
            <div id="ea-dh-top-target" style="color:#cba6f7;font-size:10px;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">Top Target: -</div>
    ...
    updateHUD() {
      ...
      const topTargetEl = document.getElementById('ea-dh-top-target');
      ...
      if (topTargetEl && this.lastMutationBreakdown?.topTargets?.length > 0) {
        const top = this.lastMutationBreakdown.topTargets[0];
        topTargetEl.textContent = `Top: ${top.selector} (${top.count}/s)`;
      }
