/**
 * Elefante Letrado Script - UI Styles & Animations v1.0.28
 * v1.0.27: Corrigido will-change sobrecarregado no #ea-panel e elementos derivados.
 * v1.0.28: Substituída a animação eaTitleGlow (que animava text-shadow
 *          diretamente, forçando o navegador a recalcular o render/repaint
 *          do texto a cada frame da animação). O efeito de brilho agora é
 *          gerado via pseudo-elemento ::after lendo data-glow-text e animando
 *          apenas a propriedade opacity, acelerada por GPU sem repaints.
 */
  ...
  window.ElefanteUIStyles = {
    inject() {
      ...
      estiloAnimacoes.textContent = `
        ...
        /* Animação de Hover e Focus suavizada nos inputs do intervalo de auto-página */
        #ea-min-input, #ea-max-input {
          transition: border-color 0.25s ease, box-shadow 0.25s ease !important;
        }

        #ea-min-input:hover, #ea-max-input:hover {
          border-color: rgba(147, 85, 224, 0.50) !important;
        }

        #ea-min-input:focus, #ea-max-input:focus {
          border-color: rgba(175, 116, 240, 0.75) !important;
          box-shadow: 0 0 5px rgba(175, 116, 240, 0.15) !important;
        }
        ...
        /* Animação de Glow de Alta Performance (Composição GPU via Opacity) */
        #ea-drag-header strong[data-glow-text] {
          position: relative;
          display: inline-block;
        }

        #ea-drag-header strong[data-glow-text]::after {
          ...
          animation: eaTitleGlowOpacity 4s ease-in-out infinite;
          pointer-events: none;
        }

        @keyframes eaTitleGlowOpacity {
          0%, 100% { opacity: 0.35; }
          50% { opacity: 0.75; }
        }
        ...
        /* Dropdown Customizado UNIFICADO (Caixa única contínua sem vão) */
        .ea-select-container {
          position: relative;
          width: 100%;
          margin-top: 14px;
        ...
        .ea-select-trigger {
          ...
          transition: border-color 0.25s ease, background 0.25s ease, border-radius 0.3s cubic-bezier(0.16, 1, 0.3, 1);
        }
        ...
        /* Quando aberto: o gatilho perde o arredondamento inferior e se conecta à lista */
        .ea-select-container.open .ea-select-trigger {
        ...
        /* Dropdown anexado diretamente (top: 100%) como uma extensão da própria caixa */
        .ea-select-dropdown {
          position: absolute;
          top: 100%;
          left: 0;
