/**
 * Elefante Letrado Script - Módulo UI v1.0.61
 ...
 * v1.0.60: resetStatus() agora verifica se o reader está ativo antes de aplicar
 *          o status padrão. Corrige o caso em que fechar um quiz com a
 *          auto-paginação ainda rodando mostrava "Modo com IA ativa" em vez
 *          de "Navegando...".
 * v1.0.61: Removida a animação eaTitleGlow inline do título (animava
 *          text-shadow diretamente, causando repaint contínuo — ver
 *          styles.v1.0.28.js para a causa raiz completa). O título agora
 *          carrega data-glow-text para alimentar a camada de glow via
 *          ::after definida no CSS, que anima opacity em vez de text-shadow.
 */
...
(function () {
  ...
  const UIModule = {
      ...
      if (!panel) {
        ...
        panel.innerHTML = `
          <div id="ea-drag-header">
            <strong data-glow-text="Elefante Letrado Script" style="color:#af74f0;font-family:'Outfit',sans-serif;font-weight:700;font-size:16px;letter-spacing:-0.2px;">Elefante Letrado Script</strong>
    ...
    resetStatus() {
      // Antes: sempre aplicava o status padrão (baseado só em apiKey/noAI),
      // mesmo se o reader (auto-paginação) ainda estivesse ativo. Isso fazia
      // o status voltar para "Modo com IA ativa" ao fechar um quiz, quando
      // deveria voltar para "Navegando...".
      const readerModule = typeof this.runtime?.getModule === 'function'
        ? this.runtime.getModule('reader')
        : this.runtime?.modules?.get('reader');

      const isReaderActive = Boolean(readerModule && readerModule.active);
    ...
    renderConfigScreen() {
      ...
      this.renderContent(`
        <div style="display:flex;flex-direction:column;align-items:center;">
          ...
          <button id="ea-model-save" class="ea-btn-animado" style="width:100%;padding:11px;border:none;border-radius:10px;background:#a6e3a1;color:#1e1e2e;font-weight:700;font-size:14px;cursor:pointer;margin-top:6px;margin-bottom:10px;display:flex;align-items:center;justify-content:center;gap:8px;">${salvarIcon} Salvar</button>
