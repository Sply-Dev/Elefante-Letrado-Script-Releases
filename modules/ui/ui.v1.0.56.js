/**
 * Elefante Letrado Script - Módulo UI v1.0.56
 * Gerenciador de interface visual, renderizador de painel flutuante e formulários.
 * v1.0.56: Otimização de GPU (translate3d) e desligamento estrito do RAF no Drag & Drop.
 */
...
(function () {
  ...
  const UIModule = {
    ...
    async init(runtime) {
      ...
      // Escuta EVENTOS DE DOMÍNIO para atualizar a interface sem acoplamento
    ...
    createPanelShell() {
      ...
      if (!panel) {
        ...
        // Sistema de Drag & Drop Otimizado via GPU (translate3d) com Parada Estrita do RAF
        ...
        const updateDragPosition = () => {
          // Condição de Repouso/Conclusão: Desliga o RAF imediatamente ao estagnar
          if (!isDragging && Math.abs(targetDx - currentDx) < 0.1 && Math.abs(targetDy - currentDy) < 0.1) {
            currentDx = targetDx;
            currentDy = targetDy;

            // Aplicação final em left/top para consolidar a posição física e limpar o transform
            baseLeft = baseLeft + currentDx;
            baseTop  = baseTop  + currentDy;
            ...
            animFrameId = null;
            return; // Encerra o loop do RAF completamente
          }

          // Lerp suave (0.35)
          currentDx += (targetDx - currentDx) * 0.35;
          currentDy += (targetDy - currentDy) * 0.35;

          // GPU-Accelerated Transform (não provoca reflow no motor de layout do navegador)
          panel.style.transform = `translate3d(${currentDx.toFixed(1)}px, ${currentDy.toFixed(1)}px, 0px)`;
        ...
        document.addEventListener('mousemove', e => {
          ...
          // Mantém o painel dentro dos limites visíveis da janela
          if (baseLeft + dx < 0) dx = -baseLeft;
          if (baseLeft + dx > maxLeft) dx = maxLeft - baseLeft;
          if (baseTop + dy < 0) dy = -baseTop;
          if (baseTop + dy > maxTop) dy = maxTop - baseTop;
    ...
    getIcons() {
      return window.ElefanteUIIcons || this.icons || {};
    },
    ...
    updateAutoButton(active) {
      ...
      if (autoBtn) {
        ...
        const i = this.getIcons();
        const playIcon = i.svgPlay || '';
        const stopIcon = i.svgStop || '';
        if (active) {
          autoBtn.innerHTML = `${stopIcon} Parar`;
          autoBtn.style.background = '#f38ba8';
        } else {
          autoBtn.innerHTML = hasKey ? `${playIcon} Iniciar` : `${playIcon} Iniciar Auto-Página`;
    ...
    renderSetupScreen() {
      const i = this.getIcons();
      const chaveIcon = i.svgChave || '';
      const olhoFechadoIcon = i.svgOlhoFechado || '';

      this.renderContent(`
        <b style="color:#cba6f7;font-family: 'Manrope', sans-serif;font-weight: 700;font-size:16px;display:flex;align-items:center;gap:8px;">${chaveIcon} Chave da API</b>
        ...
        <div style="position: relative; width: 100%; box-sizing: border-box; margin: 0 0 6px 0; transform: translateY(-7px);">
          ...
          " title="Mostrar/Ocultar chave">${olhoFechadoIcon}</button>
        </div>
    ...
    renderMainScreen() {
      ...
      const playIcon = i.svgPlay || '';
      const stopIcon = i.svgStop || '';
      const reconfigIcon = i.svgReconfigurar || '';
      const livroIcon = i.svgLivro || '';
      const configIcon = i.svgConfig || '';

      const buttonText = isRunning
        ? `${stopIcon} Parar`
        : (apiKey ? `${playIcon} Iniciar` : `${playIcon} Iniciar Auto-Página`);
      ...
      this.renderContent(`
        <div style="display:flex;justify-content:space-between;align-items:flex-start;">
          <b style="color:#cba6f7;font-family: 'Plus Jakarta Sans', sans-serif;font-size:15px;letter-spacing: 0.3px;font-weight:600;display:flex;align-items:center;gap:8px;">${livroIcon} ${bookTitle}</b>
          <button id="ea-config-btn" class="ea-btn-icone ea-config-btn" title="Configurações" style="background:#313244;border:none;border-radius:8px;color:#cdd6f4;cursor:pointer;padding:6px;display:flex;align-items:center;justify-content:center;">${configIcon}</button>
        </div>
        <div id="ea-status" style="font-size:14px;color:${statusColor};font-family:'Nunito Sans', sans-serif;font-weight:500;letter-spacing:0.2px;transform:translateY(-1px);">${statusText}</div>

        <button id="ea-auto-btn" class="ea-btn-animado" style="
          width:100%;padding:10px 11px;border:none; font-family: 'Manrope', sans-serif;
          font-weight:700; border-radius:10px;background:${buttonBg};
          font-size:14px;cursor:pointer;color:#1e1e2e; margin-top:1px; margin-bottom:0px;
        ...
        <button id="ea-reset-btn" class="ea-btn-animado" style="
          ...
          display:flex;align-items:center;justify-content:center;gap:8px;
        ">${reconfigIcon} Reconfigurar</button>
      `);
    ...
    renderConfigScreen() {
      ...
      const diagMode = (storage.getDiagMode && storage.getDiagMode()) || 'OFF';
      const diagHUD = (storage.getDiagHUD && storage.getDiagHUD()) || false;
      ...
      const configIcon = i.svgConfig || '';
      const salvarIcon = i.svgSalvar || '';
      const voltarIcon = i.svgVoltar || '';

      this.renderContent(`
        <div style="display:flex;flex-direction:column;align-items:center;">
          <b style="color:#cba6f7;font-family:'Manrope', sans-serif;font-weight:700;letter-spacing:0.2px;font-size:16px;display:flex;align-items:center;gap:8px;">${configIcon} Configurações</b>
          ...
          <div style="width:100%;border-top:1px solid #313244;margin:14px 0 10px 0;"></div>

          <p style="margin:0 0 8px;font-size:15px;color:#a6adc8;width:100%;text-align:left;font-family: 'Inter', sans-serif;font-weight: 600;letter-spacing:0.3px;">Intervalo de auto-página (minutos):</p>
          ...
          <div id="ea-interval-err" class="ea-error-box" style="width:100%;color:#f38ba8;font-size:12px;line-height:1.2;transform:translateY(-7px);opacity:0.90;"></div>

          <div style="width:100%;border-top:1px solid #313244;margin:12px 0 10px 0;"></div>

          <!-- Diagnóstico de Performance -->
          <p style="margin:0 0 8px;font-size:14px;color:#a6adc8;width:100%;text-align:left;font-family:'Inter',sans-serif;font-weight:600;">📊 Monitor de Desempenho:</p>
          
          <div style="display:flex;align-items:center;justify-content:space-between;width:100%;margin-bottom:8px;">
            <span style="font-size:12px;color:#cdd6f4;font-family:'Inter',sans-serif;">Modo:</span>
            <div style="display:flex;gap:4px;">
              <button id="ea-diag-off" style="padding:4px 8px;border:1px solid #45475a;border-radius:6px;background:${diagMode === 'OFF' ? '#f38ba8' : '#11111b'};color:${diagMode === 'OFF' ? '#1e1e2e' : '#cdd6f4'};font-size:11px;font-weight:600;cursor:pointer;">OFF</button>
              <button id="ea-diag-norm" style="padding:4px 8px;border:1px solid #45475a;border-radius:6px;background:${diagMode === 'NORMAL' ? '#89b4fa' : '#11111b'};color:${diagMode === 'NORMAL' ? '#1e1e2e' : '#cdd6f4'};font-size:11px;font-weight:600;cursor:pointer;">Normal</button>
              <button id="ea-diag-deep" style="padding:4px 8px;border:1px solid #45475a;border-radius:6px;background:${diagMode === 'DEEP' ? '#cba6f7' : '#11111b'};color:${diagMode === 'DEEP' ? '#1e1e2e' : '#cdd6f4'};font-size:11px;font-weight:600;cursor:pointer;">Profundo</button>
            </div>
          </div>

          <div style="display:flex;align-items:center;justify-content:space-between;width:100%;margin-bottom:10px;">
            <span style="font-size:12px;color:#cdd6f4;font-family:'Inter',sans-serif;">Painel HUD:</span>
            <button id="ea-diag-hud-btn" style="padding:4px 10px;border:1px solid #45475a;border-radius:6px;background:${diagHUD ? '#a6e3a1' : '#313244'};color:${diagHUD ? '#1e1e2e' : '#cdd6f4'};font-size:11px;font-weight:600;cursor:pointer;">${diagHUD ? 'Exibindo' : 'Oculto'}</button>
          </div>

          <div style="display:flex;gap:6px;width:100%;margin-bottom:12px;">
            <button id="ea-diag-export-btn" style="flex:1;padding:7px;border:none;border-radius:8px;background:#313244;color:#89b4fa;font-size:11px;font-weight:600;cursor:pointer;">💾 Exportar JSON</button>
            <button id="ea-diag-clear-btn" style="flex:1;padding:7px;border:none;border-radius:8px;background:#313244;color:#f38ba8;font-size:11px;font-weight:600;cursor:pointer;">🗑️ Limpar</button>
          </div>

          <button id="ea-model-save" class="ea-btn-animado" style="width:100%;padding:11px;border:none;border-radius:10px;background:#a6e3a1;color:#1e1e2e;font-weight:700;font-size:14px;cursor:pointer;margin-top:2px;margin-bottom:10px;display:flex;align-items:center;justify-content:center;gap:8px;">${salvarIcon} Salvar</button>
          <button id="ea-model-back" class="ea-btn-animado" style="width:100%;padding:10px;border:none;border-radius:10px;background:#45475a;color:#cdd6f4;font-size:13px;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;">${voltarIcon} Voltar</button>
        </div>
      `);
      ...
      const btnOff = document.getElementById('ea-diag-off');
      const btnNorm = document.getElementById('ea-diag-norm');
      const btnDeep = document.getElementById('ea-diag-deep');
      const btnHud = document.getElementById('ea-diag-hud-btn');
      const btnExport = document.getElementById('ea-diag-export-btn');
      const btnClear = document.getElementById('ea-diag-clear-btn');

      if (btnOff) btnOff.onclick = () => this.runtime.events.sendCommand('command:diag:set-mode', { mode: 'OFF' });
      if (btnNorm) btnNorm.onclick = () => this.runtime.events.sendCommand('command:diag:set-mode', { mode: 'NORMAL' });
      if (btnDeep) btnDeep.onclick = () => this.runtime.events.sendCommand('command:diag:set-mode', { mode: 'DEEP' });
      if (btnHud) btnHud.onclick = () => this.runtime.events.sendCommand('command:diag:toggle-hud');
      if (btnExport) btnExport.onclick = () => this.runtime.events.sendCommand('command:diag:export');
      if (btnClear) btnClear.onclick = () => this.runtime.events.sendCommand('command:diag:clear');

      if (minInput) minInput.addEventListener('input', () => intervalErr?.classList.remove('show'));
      if (maxInput) maxInput.addEventListener('input', () => intervalErr?.classList.remove('show'));
      if (customInput) customInput.addEventListener('input', () => intervalErr?.classList.remove('show'));
      ...
      if (trigger && container) {
        ...
        optionEls.forEach(opt => {
          opt.onclick = (e) => {
            ...
            container.classList.remove('open');
            intervalErr?.classList.remove('show');
      ...
      if (saveBtn) {
        saveBtn.onclick = () => {
          ...
          if (modeloFinal === '__custom__') {
            ...
            if (!modeloFinal) {
              if (intervalErr) {
                intervalErr.textContent = 'Digite o nome do modelo customizado.';
                intervalErr.classList.add('show');
              }
              return;
            }
          }

          const minVal = parseFloat(minInput?.value.replace(',', '.') || '0');
          const maxVal = parseFloat(maxInput?.value.replace(',', '.') || '0');

          if (isNaN(minVal) || isNaN(maxVal) || minVal < 0.5 || maxVal > 60 || minVal >= maxVal) {
            if (intervalErr) {
              intervalErr.textContent = 'Insira intervalos válidos (mín 0.5, máx 60).';
              intervalErr.classList.add('show');
            }
            return;
          }
